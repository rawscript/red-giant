...
func runAdaptiveTests() {
	...
	fmt.Println("═══════════════════════════════════════")
	fmt.Printf("%-15s %-10s %-12s %-10s %-10s %-12s %-10s\n",
		"Format", "Size", "Throughput", "Time(ms)", "Req(ms)", "ChunkSize", "Compressed")
	fmt.Println(strings.Repeat("-", 85))

	totalThroughput := 0.0
	for _, result := range results {
		fmt.Printf("%-15s %-10d %-12.2f %-10d %-10d %-12s %-10v\n",
			result.Format,
			result.Size,
			result.Throughput,
			result.ProcessTime,
...
// This function allows the file to be run directly if needed
// It won't conflict with the main package's main function
func main() {
	// Uncomment the line below to run the tests when this file is executed directly
	// runAdaptiveTests()
}
