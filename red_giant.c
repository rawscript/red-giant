...
rg_exposure_surface_t *rg_create_surface(const rg_manifest_t *manifest) {
  ...
  rg_exposure_surface_t *surface =
      safe_aligned_alloc(RG_CACHE_LINE_SIZE, sizeof(rg_exposure_surface_t));
  if (!surface) {
...
bool rg_expose_chunk_fast(rg_exposure_surface_t *surface, uint32_t chunk_id,
                          const void *data, uint32_t size) {
  if (!surface || !data || chunk_id >= surface->manifest.total_chunks ||
      size == 0) {
    return false;
  }
  ...
  if (size > surface->manifest.chunk_size) {
    fprintf(stderr, "[ERROR] Chunk %u size %u exceeds maximum %u\n", chunk_id,
            size, surface->manifest.chunk_size);
    return false;
  }
...
const rg_chunk_t *rg_peek_chunk_fast(rg_exposure_surface_t *surface,
                                     uint32_t chunk_id) {
...
bool rg_pull_chunk_fast(rg_exposure_surface_t *surface, uint32_t chunk_id,
                        void *dest_buffer, uint32_t *size) {
...
uint32_t rg_expose_batch(rg_exposure_surface_t *surface, uint32_t start_chunk,
                         uint32_t count, const void **data_ptrs,
                         uint32_t *sizes) {
...
uint32_t rg_pull_batch(rg_exposure_surface_t *surface, uint32_t start_chunk,
  ...
  uint32_t pulled = 0;
  for (uint32_t i = 0; i < count; i++) {
    if (dest_buffers[i] && rg_pull_chunk_fast(surface, start_chunk + i,
                                              dest_buffers[i], &sizes[i])) {
      pulled++;
    }
  }
...
uint64_t rg_get_performance_stats(const rg_exposure_surface_t *surface,
